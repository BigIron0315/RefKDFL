...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='cifar100', choices=['cifar100', 'cifar10', 'mnist'], help='dataset')
...
def main():
    ...
    with open('{file_path}'.format(file_path=os.path.join(dir_path, file_name)), 'w') as csvfile:
        ...
        if opt.dataset == 'cifar100':
            train_loader, val_loader = get_cifar100_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers, dataRatio = opt.dataRatio)
            n_cls = 100
            ...
            train_loader, val_loader = get_mnist_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers, dataRatio = opt.dataRatio)
            n_cls = 10
        else:
