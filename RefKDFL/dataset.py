...
class DatasetObject:
    def __init__(self, dataset, n_client, rule, public, unbalanced_sgm=0, rule_arg=''):
        self.dataset  = dataset
        self.n_client = n_client
        self.rule     = rule
        self.rule_arg = rule_arg        
        self.publicRatio = public
        rule_arg_str  = rule_arg if isinstance(rule_arg, str) else '%.3f' % rule_arg
        self.name = "%s_%d_%s_public_%.2f_%s" %(self.dataset, self.n_client, self.rule, self.publicRatio, rule_arg_str)
    ...
    def set_data(self):
        ...
        if not os.path.exists('%s/%s' %(self.data_path, self.name)):
            ...
            if self.dataset == 'CIFAR10':
                ...
                tst_load = torch.utils.data.DataLoader(tstset, batch_size=10000, shuffle=True, num_workers=1)
            ...
            ###
            public_ratio = self.publicRatio
            n_data_server = int((len(trn_y)) * public_ratio)
            n_data_per_clnt = int((len(trn_y) - n_data_server) / self.n_client)
            if self.unbalanced_sgm != 0:
                ...
                server_data_list = (np.ones(1) * n_data_server).astype(int)
            ...
            if self.rule == 'Dirichlet':
                ...
                print('========= idx list {} cls_amount {}'.format(idx_list, cls_amount))
                ...
                server_x = [ np.zeros((server_data_list[0], self.channels, self.height, self.width)).astype(np.float32) ]
                server_y = [ np.zeros((server_data_list[0], 1)).astype(np.int64) ]   

                for i in range(self.n_cls):
                    cls_label = i
                    data_per_cls = int(n_data_server / self.n_cls)
                    print("====cls label : ", cls_label, np.sum(clnt_data_list))
                    for j in range(data_per_cls):
                        server_x[0][cls_label*data_per_cls+j] = trn_x[idx_list[cls_label][-j]]
                        server_y[0][cls_label*data_per_cls+j] = trn_y[idx_list[cls_label][-j]]
                        cls_amount[cls_label] -= 1

                while(np.sum(clnt_data_list) !=0):
                    curr_clnt = np.random.randint(self.n_client)
                    # If current node is full resample a client
                    #print('Remaining Data: ', np.sum(clnt_data_list), np.sum(server_data_list), curr_clnt, clnt_data_list[curr_clnt])
                ...
                print('---------clnt_data_list_cum_sum', clnt_data_list_cum_sum)
                ...
                server_x[0] = trn_x[-n_data_server:]
                server_y[0] = trn_y[-n_data_server:]
            ...
            self.server_x = server_x; self.server_y = server_y
            ...
            np.save('%s/%s/server_x.npy' %(self.data_path, self.name), server_x)
            np.save('%s/%s/server_y.npy' %(self.data_path, self.name), server_y)
            ...
            self.server_x = np.load('%s/%s/server_x.npy' %(self.data_path, self.name), allow_pickle=True)
            self.server_y = np.load('%s/%s/server_y.npy' %(self.data_path, self.name), allow_pickle=True)
