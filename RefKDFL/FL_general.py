...
# --- Train functions
def train_KDFL_model(model, clnt, TA_model, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name, Temperature, isdistill, com_round, opt=None):
    n_trn = trn_x.shape[0]
    ...
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_trn/batch_size))):
            ...
            if isFirst == True:
                ...
                logit.detach_()
                logit_t = logit.detach()                              
                train_KDFL_model.logitTA[clnt].append(logit_t)
            else:
                logit_t = train_KDFL_model.logitTA[clnt]            
